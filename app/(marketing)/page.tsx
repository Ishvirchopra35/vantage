...
const STEPS = [
  ...
  { n: 2, title: 'Paste any job', description: 'Drop in a job URL or paste the description. We parse it and tailor everything instantly.' },
...
export default function LandingPage() {
  return (
    <>
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'var(--bg)', zIndex: -1, pointerEvents: 'none' }} />

      <section style={{ maxWidth: '1120px', margin: '0 auto', padding: '88px 24px 96px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: 999, padding: '7px 12px', fontSize: '0.83rem', marginBottom: 22 }}>
          <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--muted)', flexShrink: 0 }} />
          Built for students in recruiting season
        </div>

        <h1 style={{ fontSize: 'clamp(2.2rem, 6vw, 4.6rem)', lineHeight: 1.02, letterSpacing: '-0.04em', maxWidth: 840, margin: '0 0 20px 0', color: 'var(--text)' }}>
          Applied to 100 jobs. <br/> Heard back from 1. <br/> There's a better way.
        </h1>

        <p style={{ marginTop: 20, color: 'var(--muted)', maxWidth: 560, fontSize: '1.1rem', lineHeight: 1.6 }}>
        ...
        <div style={{ marginTop: 32, display: 'flex', gap: 12, alignItems: 'center' }}>
          <Link href="/signup" style={{ background: 'var(--accent)', color: 'var(--bg)', padding: '12px 20px', borderRadius: 10, fontWeight: 600, fontSize: '0.95rem', textDecoration: 'none' }}>Get started free</Link>
      ...
      <section id="sound-familiar" style={{ maxWidth: 1120, margin: '0 auto', padding: '90px 24px' }}>
        <div style={{ fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.16em', color: 'var(--muted)', marginBottom: 20 }}>SOUND FAMILIAR?</div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {PAIN_POINTS.map((text, i) => (
            <div key={i} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 22, color: 'var(--text)', fontSize: '0.98rem', lineHeight: 1.65 }}>{text}</div>
          ))}
        </div>
      </section>

      <section id="features" style={{ maxWidth: 1120, margin: '0 auto', padding: '90px 24px' }}>
        <div style={{ fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.16em', color: 'var(--muted)', marginBottom: 20 }}>EVERYTHING YOU NEED TO GET HIRED</div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
          {FEATURES.map((feature, i) => (
            <div key={i} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 22 }}>
              <div style={{ width: 32, height: 32, background: 'var(--feature-icon-bg)', borderRadius: 8, display: 'grid', placeItems: 'center', marginBottom: 14, color: 'var(--muted)' }}>{getFeatureIcon(feature.name)}</div>
              <div style={{ fontSize: '0.95rem', fontWeight: 600, marginBottom: 6, color: 'var(--text)' }}>{feature.name}</div>
              <div style={{ fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.6 }}>{feature.description}</div>
            </div>
          ))}
        </div>
      </section>

      <section id="how-it-works" style={{ maxWidth: 1120, margin: '0 auto', padding: '90px 24px' }}>
        <div style={{ fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.16em', color: 'var(--muted)', marginBottom: 20 }}>HOW IT WORKS</div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {STEPS.map((step, i) => (
            <div key={i} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 28 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', border: '1px solid var(--border)', display: 'grid', placeItems: 'center', fontSize: '0.8rem', color: 'var(--muted)', marginBottom: 16 }}>{step.n}</div>
              <div style={{ fontSize: '1rem', fontWeight: 600, marginBottom: 8, color: 'var(--text)' }}>{step.title}</div>
              <div style={{ fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.6 }}>{step.description}</div>
            </div>
          ))}
        </div>
      </section>

      <section id="pricing" style={{ maxWidth: 1120, margin: '0 auto', padding: '90px 24px' }}>
        <div style={{ fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.16em', color: 'var(--muted)', marginBottom: 20, textAlign: 'center' }}>SIMPLE PRICING</div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', maxWidth: 780, margin: '0 auto', gap: 16 }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, padding: 28 }}>
            <div style={{ fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', marginBottom: 8 }}>Free</div>
            <div style={{ fontSize: '2.4rem', fontWeight: 700, marginBottom: 20, color: 'var(--text)' }}>$0 <span style={{ fontSize: '1rem', color: 'var(--muted)' }}>/month</span></div>

            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {FREE_FEATURES.map((feature, i) => (
                <li key={i} style={{ fontSize: '0.9rem', color: 'var(--muted)', paddingLeft: 20, position: 'relative' }}>
                  <span style={{ color: 'var(--score-green)', position: 'absolute', left: 0 }}>✓</span>
                  {feature}
                </li>
              ))}
            </ul>
        ...
        <p style={{ fontSize: '0.88rem', color: 'var(--muted)', textAlign: 'center', marginTop: 16 }}>Free for the first 50 users during early access</p>
      </section>

      <section style={{ maxWidth: 1120, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 700, letterSpacing: '-0.03em', marginBottom: 12, color: 'var(--text)' }}>Stop sending the same resume to every job.</h2>
        <p style={{ color: 'var(--muted)', fontSize: '1rem', marginBottom: 32 }}>Join students who are applying smarter.</p>
        <Link href="/signup" style={{ background: 'var(--accent)', color: 'var(--bg)', padding: '12px 20px', borderRadius: 10, fontWeight: 600, fontSize: '0.95rem', textDecoration: 'none', display: 'inline-block' }}>Get started free</Link>
      </section>
    </>
  );
}
