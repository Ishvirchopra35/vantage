...
- Auto-apply: fills job application forms for you. Three ways: (1) the Chrome extension (recommended) fills the form on the page after you click a button - it never submits, you always review and submit yourself; (2) a console snippet you paste into the browser; (3) an answers panel that generates all the answers for you to copy. The extension is called "Vantage Auto-Fill" and installs from the Chrome Web Store: https://chromewebstore.google.com/detail/vantage-auto-fill/mgapanbbaplohlojbmghoglmpfpogook - then pair it using the connection code from the bottom of the Profile page. Each of the three ways uses one monthly auto-apply credit on the free plan.
